...
void	Snd_CallFunction( void (*aFunc)(), U16 tuneNumber );
...
void	Snd_TuneInit( sSndInfo * apInfo, U16 tuneNumber )
{
	sMfpTimer	lTimer;
	...
	if( apInfo )
	{
		...
		Snd_CallFunction( apInfo->mStart, tuneNumber );
...
void	Snd_TuneDeInit( sSndInfo * apInfo )
{
	if( apInfo )
	{
		...
		Snd_CallFunction( apInfo->mStop, 1 );
	}
}
